...
void solve() {
    string str, p, t; cin >> str >> p >> t;
    cout << str << endl;

    if (p.length() > str.length()) cout << str << endl;
    else {
        for (int i = p.length() - 1; str[i]; i++) {
            string tmp = str.substr(i - p.length() + 1, p.length());
            if (tmp == p) {
                cout << str.substr(0, i - p.length() + 1);
                cout << t;
