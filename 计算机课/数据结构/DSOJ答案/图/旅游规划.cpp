...
int dis[MAXN], cost[MAXN];  // div[u]表示节点到起点S的距离、花费
bool state[MAXN];  // 记录每个节点是否在队列中

pii spfa() {
    memset(dis, INF, so(dis));
    memset(cost, INF, so(cost));
    ...
    while (que.size()) {
        ...
        for (auto& [v, w, c] : edges[t]) {
            if (dis[v] > dis[t] + w || (dis[v] == dis[t] + w && cost[v] > cost[t] + c)) {
                dis[v] = dis[t] + w, cost[v] = cost[t] + c;
...
void solve() {
    ...
    while (m--) {
        ...
        edges[u].push_back({ v,w,c }), edges[v].push_back({ u,w,c });
    }
