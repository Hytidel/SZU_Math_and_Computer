...
namespace Topological_Sorting {
    ...
    int in[MAXN];  // 节点的入度
    int que[MAXN];  // 队列
    int early[MAXN], late[MAXN];  // 记录事件的最早、最晚开始时间

    bool topo() {
        int hh = 0, tt = -1;
        for (int i = 0; i < n; i++)  // 所有入度为0的节点入队
            if (!in[i]) que[++tt] = i;
...
void solve() {
    ...
    crucialPath();
    for (int i = 0; i < n; i++) cout << early[i] << ' ';
    cout << endl;
    for (int i = 0; i < n; i++) cout << late[i] << ' ';
}
