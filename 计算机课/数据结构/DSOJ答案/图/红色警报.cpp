...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        merge(cities[i].first, cities[i].second);
    }

    int cur = 0;  // 当前连通分量的个数
    for (int i = 0; i < n; i++) cur += fa[i] == i;
    ...
    for (int i = 0; i < k; i++) {
        ...
        init();
        for (int i = 0; i < m; i++) {
            if (!die[cities[i].first] && !die[cities[i].second])
                merge(cities[i].first, cities[i].second);
        }

        int cnt = 0;  // 连通分量的个数
        for (int i = 0; i < n; i++) cnt += fa[i] == i && !die[i];

        if (cnt != cur && cnt != cur - 1) cout << "Red Alert: City " << x << " is lost!" << endl;
