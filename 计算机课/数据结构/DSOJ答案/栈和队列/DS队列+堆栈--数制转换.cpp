...
umap<int, char> mp = { {10,'A'},{11,'B'},{12,'C'},{13,'D'},{14,'E'},{15,'F'} };

void solve() {
    double a; int k; cin >> a >> k;

    int tmp = (int)a;
    stack<char> stk;
    while (tmp) {
        int t = tmp % k;
        if (t < 10) stk.push(t + '0');
        else stk.push(mp[t]);
        tmp /= k;
    }
    ...
    ans.push_back('.');
    int begin = ans.length();
    
    double last = a - (int)a;
    for (int i = 0; i < 10; i++) {
        int t = last * k;
        if (t < 10) ans.push_back(t + '0');
        else ans.push_back(mp[t]);
        last = last * k - t;
    }
    cout << ans.substr(0, begin + 3) << endl;
}
