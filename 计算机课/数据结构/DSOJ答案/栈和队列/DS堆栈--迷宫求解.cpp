...
void solve() {
    ...
    while (stk.size()) {
        ...
        stk.push({ tmp.x,tmp.y,tmp.dir + 1 });

        int curx = tmp.x + dx[tmp.dir], cury = tmp.y + dy[tmp.dir];
        if (curx < 1 || curx > n || cury < 1 || cury > n || vis[curx][cury] || !maze[curx][cury]) continue;
        Node nxt = { curx,cury,0 };
        stk.push(nxt);
        pre[nxt] = tmp;
    }
