...
void solve() {
	...
	while (n--) {
		string s; cin >> s;
		vi preS(s.length() + 1), preX(s.length() + 2);  // 前缀S、X的个数
		vi pre(s.length() + 1);  // S为+1,X为-1
		for (int i = 1; i <= s.length(); i++) {
			preS[i] = preS[i - 1] + (s[i - 1] == 'S');
			preX[i] = preX[i - 1] + (s[i - 1] == 'X');
			pre[i] = pre[i - 1] + (s[i - 1] == 'S' ? 1 : -1);
		}

		bool ok = !pre[s.length()];
		for (int i = 1; i <= s.length(); i++) {
			ok &= preS[i] >= preX[i];
			ok &= pre[i] <= m;
		}
