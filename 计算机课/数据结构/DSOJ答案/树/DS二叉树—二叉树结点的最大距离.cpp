...
namespace BinaryTree {
    ...
    void floyd() {
        ...
        for (int i = 1; i <= idx; i++)
            if (i != fa[i]) dis[i][fa[i]] = dis[fa[i]][i] = 1;
    ...
    int check() {  // 判断树的形态
        for (int i = 1; i <= idx; i++)
            if (Tree[i].l && Tree[i].r) return 0;  // 普通树

        if (Tree[1].r) return 1;  // 右链
        else return 2;  // 左链
    }
};
...
void solve() {
    ...
    if (check() < 2) cout << ans << ':' << Tree[u].data << ' ' << Tree[v].data << endl;
    else cout << ans << ':' << Tree[v].data << ' ' << Tree[u].data << endl;
}
