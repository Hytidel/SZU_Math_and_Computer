...
namespace BinaryTree {
    ...
    string getArray() {
        string res(1 << idx, '#');
        res[1] = Tree[1].data, mp[Tree[1].data] = 1;
        for (int i = 1; i <= idx; i++) {
            if (Tree[i].l) {
                res[mp[Tree[i].data] << 1] = Tree[Tree[i].l].data;
                mp[Tree[Tree[i].l].data] = mp[Tree[i].data] << 1;
            } 
            if (Tree[i].r) {
                res[mp[Tree[i].data] << 1 | 1] = Tree[Tree[i].r].data;
                mp[Tree[Tree[i].r].data] = mp[Tree[i].data] << 1 | 1;
            } 
        }
...
void solve() {
    ...
    string ans; cin >> ans >> s;
    n = strlen(s);

    build();

    cout << (getArray() == ans ? "YES" : "NO") << endl;
}
