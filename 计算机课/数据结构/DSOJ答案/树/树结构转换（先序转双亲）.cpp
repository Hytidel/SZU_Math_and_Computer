...
namespace BinaryTree {
    ...
    int build() {  // 建树
        ...
        if (Tree[cur].l) fa[Tree[cur].l] = cur;
        if (Tree[cur].r) fa[Tree[cur].r] = cur;
        return cur;
    }
    ...
    void get() {
        arr[1] = 1;
        for (int i = 1; i <= idx; i++) {
            if (Tree[i].l) arr[i << 1] = Tree[i].l;
            if (Tree[i].r) arr[i << 1 | 1] = Tree[i].r;
        }
    }
};
...
void solve() {
    ...
    map<int, int> mp;
    int idx = 0;
    for (int i = 1; i <= (1 << idx + 1); i++) {
        if (!arr[i]) continue;

        ans.push_back(Tree[arr[i]].data), res.push_back(arr[i]);
        mp[arr[i]] = idx++;
    }
    for (int i = 0; i < ans.size(); i++) cout << ans[i] << " \n"[i == ans.size() - 1];
    cout << "-1 ";
    for (int i = 1; i < ans.size(); i++) cout << mp[fa[res[i]]] << " \n"[i == ans.size() - 1];
}
