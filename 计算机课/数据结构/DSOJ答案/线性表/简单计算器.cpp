...
// umap<char, int> pro = { {'+',1},{'-',1},{'*',2},{'/',2},{'^',3} };  // 运算符优先级
umap<char, int> pro = { {'+',1},{'-',1},{'*',1},{'/',1},{'^',1} };  // 运算符优先级
stack<int> nums;  // 操作数
stack<int> op;  // 运算符
...
void solve() {
	...
	for (int i = 0; i < line.length(); i++) {
		...
		else if (pro.count(ch)) {  // 运算符
			while (op.size() && op.top() != '(' && pro[op.top()] >= pro[ch]) {
				if (!cal()) {
					cout << "ERROR";
					return;
				}
