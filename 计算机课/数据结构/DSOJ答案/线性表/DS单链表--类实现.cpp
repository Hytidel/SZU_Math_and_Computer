...
public:
	...
	Node* FindByPos(int pos) {  // 返回第pos个节点
	...
	bool RemoveByPos(int pos) {  // 删除第pos个节点
		Node* p = FindByPos(pos - 1);  // 找到欲删除的节点的前一个节点
		if (!p || !(p->nxt)) return false;  // 不存在该节点
...
void solve() {
	...
	pos--;
	if (l.InsertToPos(val, pos)) l.print();
	...
	cin >> pos;
	auto ans = l.FindByPos(pos);
	if (ans && ans->get_data()) cout << ans->get_data() << endl;
	else cout << "error" << endl;
	cin >> pos;
	ans = l.FindByPos(pos);
	if (ans && ans->get_data()) cout << ans->get_data() << endl;
