...
bool Joystick::init(Vec2 aPoint , float aRadius , const char* aJsSprite, const char* aJsBg)
{
	if (!Layer::init())
	{
		return false;
...
Joystick* Joystick::create(Vec2 aPoint, float aRadius, const char* aJsSprite, const char* aJsBg)
{
...
Vec2 Joystick::getDirection()
{
	...
	return (m_currentPoint - m_centerPoint).getNormalized();
}
