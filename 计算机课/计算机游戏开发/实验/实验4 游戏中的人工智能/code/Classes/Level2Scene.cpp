...
// ---------= 加载场景 =---------
	...
	// 加载迷宫地图
	loadTiledMaze(MAP_WIDTH, MAP_HEIGHT, Point(visibleSize.width / 2, visibleSize.height - MAP_HEIGHT * MAP_UNIT / 2), 0, TILED_MAZE_2_PATH);  // 画面中上方
...
// 触屏结束的回调函数
void Level2Scene::onTouchEnded(Touch* touch, Event* event) {
...
void Level2Scene::loadDangerousTip() {
...
// 加载层数信息
void Level2Scene::displayLevel() {
	auto levelTip = Label::createWithBMFont(TIPS_FNT_PATH, "Level 2");
...
// 加载纯色背景
void Level2Scene::loadPureBackground() {
...
// 加载迷宫地图
void Level2Scene::loadTiledMaze(int mapWidth, int mapHeight, Point position, int z, std::string mapPath, int mapUnit, int tag) {
...
// 屏幕坐标转地图坐标
std::pair<int, int> Level2Scene::screenCoordinate2MapCoordinate(Point screenPoint) {
	return screenCoordinate2MapCoordinate(screenPoint.x, screenPoint.y);
}

// 屏幕坐标转地图坐标
std::pair<int, int> Level2Scene::screenCoordinate2MapCoordinate(float screenX, float screenY) {
...
// 地图坐标转屏幕坐标 (tile 的中心)
Point Level2Scene::mapCoordinate2ScreenCoordinate(std::pair<int, int> mapPoint) {
	return mapCoordinate2ScreenCoordinate(mapPoint.first, mapPoint.second);
}

// 地图坐标转屏幕坐标 (tile 的中心)
Point Level2Scene::mapCoordinate2ScreenCoordinate(int mapX, int mapY) {
...
// 加载 Tile (后续不用该精灵)
void Level2Scene::loadTile(std::string path, Point position, int z, bool visible, int tag) {
...
// 加载猫尾草
void Level2Scene::loadCattail(Point position, int z, bool visible, int tag) {
...
// 加载坚果墙
void Level2Scene::loadWalnut(Point position, int z, bool visible, int tag) {
...
// 加载禁止
void Level2Scene::loadForbidden(Point position, int z, bool visible, int tag) {
...
// 加载传送门
void Level2Scene::loadPortal(Point position, int z, bool visible, int tag) {
...
// 加载头颅
void Level2Scene::loadSkull(Point position, int z, bool visible, int tag) {
...
// 加载减速
void Level2Scene::loadSpeedDown(Point position, int z, bool visible, int tag) {
...
// 加载加速
void Level2Scene::loadSpeedUp(Point position, int z, bool visible, int tag) {
...
// 随机取一个可用的点
std::pair<int, int> Level2Scene::getRandomAvailablePoint() {
...
// 占用一个可用的点
void Level2Scene::occupyAvailablePoint(std::pair<int, int> point) {
	availablePoints.erase(point);
	usedAvailablePoints.insert(point);
}

// 归还一个可用的点
void Level2Scene::returnAvailablePoint(std::pair<int, int> point) {
	usedAvailablePoints.erase(point);
	availablePoints.insert(point);
}

// 恢复可用的点
void Level2Scene::recoverAvailablePoints() {
...
// 碰撞检测
void Level2Scene::collisionDetection() {
	// 猫尾草与头颅 (先判断危险)
	for (auto skull : skulls) {
