...
// ---------= Ìí¼Ó¼àÌý =---------
	addKeyboardListener();  // Ìí¼Ó¼üÅÌ¼àÌý
	addTouchListener();  // Ìí¼Ó´¥ÆÁ¼àÌý
...
// ´¥ÆÁ¿ªÊ¼µÄ»Øµ÷º¯Êý
bool BasicScene::onTouchBegan(Touch* touch, Event* event) {
...
// ´¥ÆÁ½áÊøµÄ»Øµ÷º¯Êý
void BasicScene::onTouchEnded(Touch* touch, Event* event) {
...
// Ìí¼Ó´¥ÆÁ¼àÌý
void BasicScene::addTouchListener() {
	auto listener = EventListenerTouchOneByOne::create();
	listener->onTouchBegan = CC_CALLBACK_2(BasicScene::onTouchBegan, this);
	listener->onTouchEnded = CC_CALLBACK_2(BasicScene::onTouchEnded, this);
	// ÉèÖÃ´¥Ãþ¼àÌýÓÅÏÈ¼¶
	Director::getInstance()->getEventDispatcher()->addEventListenerWithSceneGraphPriority(listener, this);
}
...
// ÏÔÊ¾ÌáÊ¾ÐÅÏ¢
void BasicScene::displayWords(std::string words, Point position, int z, Color3B color, int tag) {
	auto wordsTip = Label::createWithBMFont(TIPS_FNT_PATH, words);
	if (wordsTip) {
		wordsTip->setPosition(position);
		wordsTip->setColor(color);
		if (~tag) {
			wordsTip->setTag(tag);
		}
