...
Level2::Level2() : currentTime(0, 0, 0), DELTA_TIMER(100), END_TIME(Time(0, 15, 0)), TARGET_SCORE(10) {
}
...
void Level2::loadHitLine(std::string linePath, int y, int z) {
...
void Level2::loadNote(int type, Time time, float x) {
	if (type == -1) {
...
void Level2::createNote(float dt) {
	total_notes++;
	loadNote(-1, Time(0, 0, 0), -1);
}

void Level2::preloadSounds() {
	// 预加载音效
	SimpleAudioEngine::getInstance()->preloadEffect(HIT_SOUND_PATH.c_str());
	
	// 预加载音乐
	SimpleAudioEngine::getInstance()->preloadBackgroundMusic(INTRO_PATH.c_str());
}
...
bool Level2::init() {
	...
	// 加载面板
	loadBoard(BOARD2_PATH, 1);
	// 加载判定线
	loadHitLine(LINE_PATH, 100, 2);
	...
	// 背景音乐
	SimpleAudioEngine::getInstance()->playBackgroundMusic(INTRO_PATH.c_str());
	...
	// 加载提示信息
	loadHint(LEVEL2_HINT_PATH, LEVEL2_HINT_TAG, 10);
...
void Level2::workHit(Note& note, int state) {
...
void Level2::updateNotes(Time currentTime) {
	for (auto& note : notes) {
		// note 已到判定线
...
void Level2::displayResult() {
	loadResultBoard(LEVEL2_RESULT_PATH, 10);
	displayScore();
}
...
void Level2::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
	switch (keyCode) {
		...
		case EventKeyboard::KeyCode::KEY_SPACE: {  // 空格: 开始游戏
			this->removeChildByTag(LEVEL2_HINT_TAG);
			pause = false;
...
void Level2::replayButtonEvent(cocos2d::Ref* pSender) {
	Director::getInstance()->popScene();
	level2Scene = Level2::createScene();
	Director::getInstance()->pushScene(level2Scene);
}

void Level2::nextButtonEvent(cocos2d::Ref* pSender) {
	Director::getInstance()->popScene();
	level3Scene = Level3::createScene();
	Director::getInstance()->pushScene(level3Scene);
}