...
Level1::Level1() : currentTime(0, 0, 0), DELTA_TIMER(100), END_TIME(Time(0, 15, 0)), TARGET_SCORE(10) {
}
...
void Level1::loadHitLine(std::string linePath, int y, int z) {
...
void Level1::loadNote(int type, Time time, float x) {
	if (type == -1) {
...
void Level1::createNote(float dt) {
	total_notes++;
	loadNote(-1, Time(0, 0, 0), -1);
}

void Level1::preloadSounds() {
	// 预加载音效
	SimpleAudioEngine::getInstance()->preloadEffect(HIT_SOUND_PATH.c_str());
	
	// 预加载音乐
	SimpleAudioEngine::getInstance()->preloadBackgroundMusic(PEOPLE_IS_PATH.c_str());
}
...
bool Level1::init() {
	...
	// 背景音乐
	SimpleAudioEngine::getInstance()->playBackgroundMusic(PEOPLE_IS_PATH.c_str());
	...
	// 加载提示信息
	loadHint(LEVEL1_HINT_PATH, LEVEL1_HINT_TAG, 10);
...
void Level1::workHit(Note& note, int state) {
...
void Level1::updateNotes(Time currentTime) {
	for (auto& note : notes) {
		// note 已到判定线
...
void Level1::displayResult() {
	loadResultBoard(LEVEL1_RESULT_PATH, 10);
	displayScore();
}
...
void Level1::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
	switch (keyCode) {
		...
		case EventKeyboard::KeyCode::KEY_SPACE: {  // 空格: 开始游戏
			this->removeChildByTag(LEVEL1_HINT_TAG);
			pause = false;
...
void Level1::replayButtonEvent(cocos2d::Ref* pSender) {
	Director::getInstance()->popScene();
	level1Scene = Level1::createScene();
	Director::getInstance()->pushScene(level1Scene);
}

void Level1::nextButtonEvent(cocos2d::Ref* pSender) {
	Director::getInstance()->popScene();
	level2Scene = Level2::createScene();
	Director::getInstance()->pushScene(level2Scene);
}