#include "stdafx.h"
#include "FaceDetectFMTCNN.h"

CFaceDetectFMTCNN::CFaceDetectFMTCNN(void)
{

	//CFaceDetectFMTCNN detector("model_FMTCNN");

	factor = 0.709f;
	float temp_val[3] = { 0.7f, 0.6f, 0.6f };
	for (int i = 0; i < 3;i++)
	{
		threshold[i] = temp_val[i];
	}
...
//CFaceDetectFMTCNN::CFaceDetectFMTCNN(const string& proto_model_dir) {
//	//    PNet_ = cv::dnn::readNetFromCaffe(proto_model_dir + "/det1.prototxt", proto_model_dir + "/det1_half.caffemodel");
//	PNet_ = cv::dnn::readNetFromCaffe(proto_model_dir + "/det1_.prototxt", proto_model_dir + "/det1_.caffemodel");
//
//	RNet_ = cv::dnn::readNetFromCaffe(proto_model_dir + "/det2.prototxt", proto_model_dir + "/det2_half.caffemodel");
...
void CFaceDetectFMTCNN::GenerateBBox(Mat* confidence, Mat* reg_box,
	...
	//std::cout << confidence->size;       //2019.1.13 remove
	//std::cout << " " << scale << std::endl; //2019.1.13 remove

	/* //display the following information
	1 x 2 x 115 x 155 1
	1 x 2 x 81 x 109 0.709
	1 x 2 x 56 x 76 0.502681
	1 x 2 x 38 x 53 0.356401
	1 x 2 x 26 x 36 0.252688
	1 x 2 x 17 x 24 0.179156
	1 x 2 x 11 x 16 0.127022
	1 x 2 x 6 x 10 0.0900583
	1 x 2 x 3 x 6 0.0638513*/
...
std::vector<FaceInfo> CFaceDetectFMTCNN::NMS(std::vector<FaceInfo>& bboxes,
...
vector<FaceInfo> CFaceDetectFMTCNN::NextStage(const cv::Mat& image, vector<FaceInfo> &pre_stage_res, int input_w, int input_h, int stage_num, const float threshold) {
	vector<FaceInfo> res;
...
vector<FaceInfo> CFaceDetectFMTCNN::ProposalNet(const cv::Mat& img, int minSize, float threshold, float factor) {
	cv::Mat  resized;
	int width = img.cols;
	int height = img.rows;
...
vector<FaceInfo> CFaceDetectFMTCNN::Detect_mtcnn(const cv::Mat& image, const int minSize, const float* threshold, const float factor, const int stage) {
...
void CFaceDetectFMTCNN::DetectFace_FMTCNN(Mat image, vector<FaceInfo> & faceInfo)
{
	faceInfo=Detect_mtcnn(image, minSize, threshold, factor, 3);

}

void CFaceDetectFMTCNN::DispFaceBoxLandMarks(Mat image, vector<FaceInfo> faceInfo)
{
	for (int i = 0; i < faceInfo.size(); i++) //faceInfo.size(): the number of the faces
	{
		...
		//Draw Landmark on face(5 key points)
		for (int j = 0; j < 5; j++)
		{
			Point KeyPoint = cvPoint((int)faceInfo[i].landmark[2 * j], (int)faceInfo[i].landmark[2 * j + 1]);
			cv::circle(image, KeyPoint, 2, CV_RGB(255, 0, 0), 2, 8, 0);


			//printf("%d-th Keypoint:(%d,%d)\n", j + 1, KeyPoint.x, KeyPoint.y);
		}
	}
